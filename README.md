## Olá, meu nome é Isaac Hermel Reginato

Sou um "guri" de 18 anos do Rio Grande do Sul 🚩 que sempre foi apaixonado por tecnologia, e
tendo o objetivo de virar um "Cara do TI" desde pequeno, aos 14 anos comecei o curso de <strong>Programação para a Internet</strong>
no <strong>IFSul - Campus Gravataí</strong>, onde tive minha introdução ao mundo da <strong>Tecnologia da Informação 👩‍💻</strong>.

Desde então, tenho estudado principalmente o <strong>Desenvolvimento Front-End</strong> e <strong>UX/UI Design</strong>, que são
as duas áreas as quais me interesso.

### Estas são minha estatísticas atuais do GitHub:

<div>
  <img height="160em" src="https://github-readme-stats.vercel.app/api?username=IsaacHermel&show_icons=true&theme=dracula&include_all_commits=true&count_private=true"/>
  <img height="160em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=IsaacHermel&layout=compact&langs_count=7&theme=dracula"/>
</div>

### Atualmente eu estou utilizando as seguintes tecnologias:

<div>
  <div>
    <h4>Linguagens:</h4>
    <div>
    </div>
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-plain.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/npm/npm-original-wordmark.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original-wordmark.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/svelte/svelte-original.svg" />
  </div>
  <div>
    <h4>Ferramentas</h4>
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" />
    <img width="48" height="48" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/canva/canva-original.svg" />
  </div>
</div>

### Você pode entrar em contato comigo por um destes meios:

<a href="mailto:isaachermel@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
<a href="https://api.whatsapp.com/send?phone=5551997099876" arget="_blank" rel="noopener noreferrer"><img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white"/></a>
<a href="https://www.linkedin.com/in/isaachermel/" target="_blank" rel="noopener noreferrer"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
<a href="https://github.com/IsaacHermel" target="_blank" rel="noopener noreferrer"><img src="	https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white"/></a>

### Perguntas que você pode ter sobre mim:

- <strong>"No que você esta trabalhando atualmente?"</strong>: Atualmente trabalho na <a href="https://umbler.com/br" target="_blank" rel="noopener noreferrer"><strong>Umbler 🐨</strong></a>, onde atuo no time de Customer Success atendendo clientes e participando de projetos internos que envolvam desenvolvimento front-end.
- <strong>"O que você esta estudando?"</strong>: Estou no momento estudando React e React Native por meio de vídeos, documentações e cursos, desenvolvendo pequenos projetos para ganhar maior domínios das 2 tecnologias.
- <strong>"Esta disponível para novos projetos?"</strong>: Atualmente não, pois estou focando em meu Trabalho de Conclusão de Curso, pois me encontro no último ano de meu curso técnico no IFSul - Campus Gravataí.

### Outras informações:

- <strong>Pronomes</strong>: ele/dele
- <strong>Fatos interessantes</strong>: Tenho um grande medo de abelhas e consigo deslocar livremente um dos meus dedões, além disso, uso computador desde os meus 2 anos de idade, oque resultou em eu conseguir digitar em uma velocidade bem acima do normal (cerca de 130 palavras por minuto 😅).
